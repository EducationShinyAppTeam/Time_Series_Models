...
shinyServer(function(session, input, output) {
  ...
  observeEvent(input$info2,{
    sendSweetAlert(
      ...
      title = "Instructions:",
      text = "
      • The aim of the analyzing the real data part is to let you try to use the ACF,PACF and ARMAsubset plot to guess the order of the model, there are three main parts of the secion: 1.Decide the stationary. 2.guess the order of the statistics. 3.Use the order the users guessed in part 2 to make forcast and compare the forcast to the real one.\n
  ...
  observeEvent(input$phi1, {
    ...
    val <- input$phi1
    updateSliderInput(session, inputId="phi2", min=abs(val)-0.99, max=0.99-abs(val), value=input$phi2, step=.01)
  })
  #make sure higher order models are stationary
  observeEvent(input$theta1, {
    if(input$q == '1'){return(NULL)}
    val <- input$theta1
    updateSliderInput(session, inputId="theta2", min=abs(val)-0.99, max=0.99-abs(val), value=input$theta2, step=.01)
  })
  ...
  observeEvent(
    ...
    output$plotSIM <- renderPlot({
      if(input$models == "AR"){
        ...
        sim(arima.sim(n=input$n,list(order=c(2,input$d,2),ma=c(input$theta1,tryCatch(input$theta2,error=function(e)NULL)), ar=c(input$phi1, tryCatch(input$phi2, error=function(e)NULL)))))
      }
 ...
 observeEvent(
  ...
  output$subsets <- renderPlot({
    transformed_data <- transformed()
    plot(armasubsets(transformed_data, nar=9, nma=9), draw.grid = FALSE,col=gray(0:1500/2000))
  })
