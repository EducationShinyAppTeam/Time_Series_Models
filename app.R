        ...
        tabItem(
          ...
          h1("Time Series Model"),
            ...
            h3(tags$b("Instructions:")),
                ...
                column(11,
                       h4(tags$li("Use the sliders for the coefficients and explore how changing parameter values affects the time series plot.")),
                       h4(tags$li("Use the drop down menus and observe how different orders of models effect the autocorrelation function (ACF) and partial autocorrelation function (PACF) plots."))
                ),
                fluidRow(
                  br()
                ),
                ...
                column(11,
                       h4(tags$li("In the first tab, select the data set that you would like to analyze, and fit transformations until the data seems stationary.")),
                       h4(tags$li("Here, you must consider that the last 12 observations of each data set are hidden from the user so that they could be presented in the last tab alongside the user's model's forecasts.")),
                       h4(tags$li("For each transformation consider the following:"))
                ),
                column(9, offset=1,
                       h4(tags$li("The transformations are of the form `seas_diff(diff(log(data)))`, that is, the log transformation will always be taken first, followed by the difference of lag one, and then the seasonal differencing.")),
                       h4(tags$li("The trend can only be removed before any other transformation, or after all transformations. The trend is removed using regression, and the transformed data is the residuals from that regression."))
                ),
                fluidRow(
                  br()
                ),
                div(style = "text-align: center",
                    bsButton("go2", "GO!", icon("bolt"), size = "large", class="circle grow")
                ),
                ...
                column(11,
                       ...
                       h4(tags$li("You are playing as the X's, the object of the game is to get 3 X's in a row.")),
                       h4(tags$li("You win when 3 X's line up horizontally, vertically, or diagonally, and you lose when 3 O's line up horizontally, vertically or diagonally, otherwise the game results in a tie."))
                ),
                fluidRow(
                  br()
                ),
                ...
                div(style = "text-align: center",
                    bsButton("go3", "GO!", icon("bolt"), size = "large", class="circle grow")
                ),
