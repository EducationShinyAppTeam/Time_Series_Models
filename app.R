      ...
      tabItems(
        ...
        tabItem(
          ...
          p("In the three parts of the app, you will explore time series with 
            simulations, challange yourself with time series analysis, and 
            review your knowledge of associated material with a game."),
          p("In particular, the first part requires students to engage with 
            simulations so they could find out how model equations may relate 
            to the graphical representation of the time series. The second 
            feature walks the user through time series analysis of a selected 
            real world data set. The user will make the data stationary, fit a 
            ...
            tabPanel(
              ...
              sidebarLayout(
                ...
                conditionalPanel(
                  ...
                  checkboxInput(
                    inputId="trend", 
                    label="Remove trend/non-constant mean in data", value=FALSE),
                  conditionalPanel(
                ...
                fluidRow(
                  ...
                  column(6, plotOutput("fitQuality1")),
                    bsPopover(
                      ...
                      content = "The ACF and PACF of the residuals of the 
                      fitted model can indicate if theres any remaining 
                      correlation structure in the data.", 
                      trigger = "hover", placement = "top"),
                    column(6, plotOutput("fitQuality2")),
                    bsPopover(
                      id="fitQuality2", title = "PACF residuals", 
                      content = "The ACF and PACF of the residuals of the 
                      fitted model can indicate if theres any remaining 
                      correlation structure in the data.", trigger = "hover", 
                      placement = "top")
              )),
              fluidRow(
...
server <- function(input, output, session) {
  ...
  observeEvent(input$phi1, {
    ...
    val <- input$phi1
    updateSliderInput(session, inputId="phi2", 
    ...
    {input$models
      ...
      input$q}, {
        output$plotSIM <- renderPlot({
          if(input$models == "AR"){
            sim(arima.sim(n=input$n,
                          list(ar=c(input$phi1,
                                    tryCatch(input$phi2, 
                                             error=function(e)NULL)))))
          }else if(input$models == "MA"){
            sim(arima.sim(
              n=input$n,
              list(ma=c(input$theta1,
                        tryCatch(input$theta2, error=function(e)NULL)))))
          }else if(input$models == "ARMA"){
            sim(arima.sim(
              n=input$n,
              list(ma=c(
                input$theta1,
                tryCatch(input$theta2, 
                         error=function(e)NULL)), 
                ar=c(input$phi1, tryCatch(input$phi2, error=function(e)NULL)))))
          }
  ...
  # these render the same plot, but we need two different calls to comply with 
  # constraints introduced by the UI
  ...
  #server side calculations for the data and the function of the challange 
  # itself
  ...
  data <- eventReactive(input$sets, {
    if(input$sets == "Choose"){
      ...
      ts(MonthlyTrafficFatalities, 
         frequency = frequency(MonthlyTrafficFatalities), start=1960)
  ...
  observe({
    validate(
      need(
        input$trend1 == FALSE & input$trend2 == FALSE & 
          input$trend3 == FALSE & input$log == FALSE & 
          input$diff == FALSE & input$seas_diff == FALSE, message=''))
  ...
  observeEvent(
    ...
    # i say this when adding the trend2 and trend3 variables, because of this, 
    # i needed to add a validate call below
    warning(checks$a)
    warning(checks$b)
    warning(checks$c)
    #the training portion of the data,
    data <- ts(data()[1:(length(data())-12)], 
               frequency = ifelse(frequency(data())==1, input$frequency, 
                                  frequency(data())))
...
# (if it was just recently entered)
  ...
  output$original <- renderPlot({
    ...
    if(input$sets == "internet"){
      X <- read.table("internet.traffic.csv", sep=",", header = T)
      hours <- as.POSIXct(X$Time, format = "%m/%d/%Y %H:%M")
      plot(hours[1:(length(data())-12)], data()[1:(length(data())-12)], 
           type="l",main = "Hourly Internet Data (in Bits) from an ISP in the UK",
      ...
      plot.ts(ts(data()[1:(length(data())-12)], 
                 frequency = 1, start = -9823) , 
              main = "Sedimentary deposit (sand and silt from melting glaciers) 
              thicknesses every year in Massachusetts",
              ylab = ylabel(), xlab = "year")
    }else if(input$sets == "sheep"){
      plot.ts(ts(data()[1:(length(data())-12)], 
                 frequency = 1, start = 1867) , 
              main = "Sheep Population (in millions) for England and Wales",
              ylab = ylabel(), xlab = "year")
    }else if(input$sets == "soi"){
      plot.ts(ts(data()[1:(length(data())-12)], 
                 frequency = 12, start = 1950), 
              main = "Southern Oscillation Index (monthly data)",
              ylab = ylabel(), xlab = "year")
    }else if(input$sets == "temperature"){
      plot.ts(ts(data()[1:(length(data())-12)], 
                 frequency = 365, start = 2001) , 
              main = "Daily Maximum Temperature in State College, Pa",
              ylab = ylabel(), xlab = "year")
    }else{
  ...
  observeEvent(input$sets,{
    ...
    citation <- if(input$sets == "internet"){
      "https://datamarket.com/data/set/232h/internet-traffic-data-in-bits-from-
      an-isp-aggregated-traffic-in-the-united-kingdom-academic-network-backbone-
      ...
      "https://datamarket.com/data/set/22ty/monthly-traffic-fatalities-in-
      ontario-1960-1974#!ds=22ty&display=line"
    }else if(input$sets == "varve"){
      "http://anson.ucdavis.edu/~shumway/tsa.html"
    }else if(input$sets == "sheep"){
      "https://datamarket.com/data/set/22px/annual-sheep-population-1000s-in-
      england-wales-1867-1939#!ds=22px&display=line"
    }else if(input$sets == "soi"){
    ...
    output$cite <- renderUI({
      addPopover(session = session, id="original", 
                 title="Citation", 
                 content=sprintf("Many of these data sets were taken from the 
                                 text book used in PSU's time series classes 
                                 (Time series analysis and its applications, 
                                 Stoffer and Shumway). %s", 
                                 ifelse(is.na(citation), "", 
                                        sprintf("Further documentation can be 
                                                found here: %s", citation))), 
                 placement = "left")
    })
  })
  ...
  output$transform <- renderPlot({
    ...
    validate(need(!(
      input$trend1 == FALSE & input$trend2 == FALSE & input$trend3 == FALSE & 
        input$log == FALSE & input$diff == FALSE & input$diff2 == FALSE & 
        input$seas_diff == FALSE), 
      message='Choose a transformation to be displayed below'))
    warning("plot transformation")
    transformed_data <- ts(transformed(), 
                           frequency = ifelse(
                             frequency(data())==1, input$frequency, 
                             frequency(data())), 
                           start = if(input$sets=="temperature"){2001}
                           else if(input$sets=="monthly"){1960}
                           else if(input$sets=="sheep"){1867}
                           else if(input$sets=="varve"){-9823}else{1})
  ...
  observeEvent(
    {input$trend
      ...
      input$trend3}, {
        ...
        warning(disabled_())
        if(disabled_()=="YES"){
          addPopover(session, id="div", title="warning", 
                     content="If you select 'remove trend' then you must select 
                     a method of trend removal", placement = "top")
  ...
  #   warning(disabled_())
  #   if(disabled_()=="YES"){
  #     "If you select 'remove trend' then you select method of trend removal"
  #   }else if(disabled_()=="NO"){
  #     "FCUCK"
  #   }else{
  #     "FUCK"
  ...
  # when the model doesnt change after we leave the 3rd tab (step3) and also 
  # still hasnt changed when the user comes back to the 3rd tab, we dont want 
  # to recalculate the same fit, so we create "counter" reactive values to 
  # account for this in the fit() reactive handler and the model() 
  # observation handler
  # {idk if this still applies i changed the counter code since}:  must keep 
  # in mind that (when both fit and model are created in a reactive handler) 
  # though we want model list (counter$a) to govern what happens, its hard when 
  # you consider the fact that it is called inside the reactive call to fit 
  # (since reactive handlers are "lazy")
  ...
  observeEvent(
    ...
    if(counter$a == counter$b){
      ...
      model(list(
        ifelse(
          input$log==TRUE,"yes","no"),
        c(input$p.order, 
          ifelse(input$diff==TRUE, 
                 ifelse(input$diff2==TRUE, 2, 1), 0), input$q.order),
        c(input$P.order, ifelse(input$seas_diff==TRUE, 
                                ifelse(input$seas_diff2==TRUE, 2, 1), 0), 
          input$Q.order),
        ifelse( (input$seas_diff | input$trend2 | input$trend3), 
                if(input$period==0){if(input$trend2 | input$trend3)
                  {input$frequency}else{input$seas}}else{input$period}, 
                input$period),
        trend)
                   )
                 })
  ...
  output$ACF <- renderPlot({
    ...
    acf(transformed_data, lag.max = 60, 
        main = "Autocorrelation Function of the Transformed data")
  })
  output$PACF <- renderPlot({
    transformed_data <- transformed()
    pacf(transformed_data, lag.max = 60, 
         main = "Partial Autocorrelation Function of the Transformed data")
  })
  ...
  fit <- reactive({
    #this arima function calculation is burdensome and time consuming, 
    # so we use validate
    ...
    if(counter$a == counter$b){
      ...
      if(input$trend1 == FALSE & input$trend2 == FALSE & input$trend3 == FALSE){
        ...
        fit.holder(tryCatch(
          arima(data, order=model()[[2]], 
                seasonal = list(order=model()[[3]], 
                                period=model()[[4]]), method = "ML"), 
          error=function(e)
          tryCatch(arima(data, order=model()[[2]], 
                         seasonal = list(order=model()[[3]], 
                                        period=model()[[4]]), 
                         method = "CSS-ML"), error=function(e)
            tryCatch(arima(data, order=model()[[2]], 
                           seasonal = list(order=model()[[3]], 
                                           period=model()[[4]]), method="CSS"))
          )
        ))
      }else{
        data <- ts(transformed(), 
        ...
        if(when()=="first"){
          ...
          if(input$trend1 == TRUE){
            ...
            fit.holder(
              arima(
                ...
                seasonal = list(order=model()[[3]], 
                                period=model()[[4]]), 
                xreg=as.numeric(reg.mean()), include.mean = T, method = "ML"))
          }else{
            #AND YOU CANT DO log() after getting residuals from regression 
            # because residuals are standardized (zero mean)
            fit.holder(
              arima(
                data, 
                order=model()[[2]], 
          ...
          if(input$trend1 == TRUE){
            fit.holder(
              arima(
                transformed(), 
                order=c(model()[[2]][1], 0, model()[[2]][3]), 
                seasonal = list(order=c(model()[[3]][1], 0, 
                                        model()[[3]][3]), period=model()[[4]]), 
                             xreg = as.numeric(reg.mean()), 
                include.mean = TRUE, method="ML"))
          }else{
            fit.holder(
              arima(
  ...
  output$forecast <- renderPlotly({
    ...
    if(input$trend1 == FALSE & input$trend2 == FALSE & input$trend3 == FALSE){
      ...
      data <- ts(data(), frequency = ifelse(frequency(data())==1, 
                                            input$frequency, frequency(data())))
      if(input$trend1 == TRUE){
        reg.mean()
        predictions <- predict(
                        fit(), 12, 
                        newxreg = data[(length(data)-11):length(data)])$pred 
      }else{
    ...
    if(when()=="last" & input$trend1==FALSE){
      ...
      #we need a manual calculation to convert the predictions if the trend 
      # was removed last because of the (transformed) data that fed to the 
      # arima fit
      data <- as.numeric( data()[1:(length(data())-12)] )
      if(input$log == TRUE){
        data <- log(data)
      ...
      if(input$diff2 == TRUE & input$seas_diff2 == TRUE){
        fore <- c('', predictions)
        fore <- diffinv(diffinv(fore, lag=input$seas, differences=2, 
                                xi=diff(data, differences=2)[1:(input$seas*2)]), 
                        differences = 2, xi=data[1:2])
      }else if(input$diff == TRUE & input$seas_diff2 == TRUE){
        fore <- c('', predictions)
        fore <- diffinv(diffinv(
                              fore, lag=input$seas, differences=2, 
                              xi=diff(data, differences=1)[1:(input$seas*2)]), 
                        differences = 1, xi=data[1])
      }else if(input$diff2 == TRUE & input$seas_diff == TRUE){
        fore <- c('', predictions)
        fore <- diffinv(diffinv(
          fore, lag=input$seas, 
          differences=1, xi=diff(data, differences=2)[1:(input$seas)]), 
          differences = 2, xi=data[1:2])
      }else if(input$diff == TRUE & input$seas_diff == TRUE){
        fore <- c('', predictions)
        fore <- diffinv(diffinv(fore, lag=input$seas, 
                                differences=1, 
                                xi=diff(data, differences=1)[1:(input$seas)]), 
                        differences = 1, xi=data[1])
      }else if(input$seas_diff == TRUE){
        if(input$seas_diff2 == TRUE){
          fore <- c('', predictions)
          fore <- diffinv(fore, lag=input$seas, 
                          differences=2, xi=data[1:(input$seas*2)])
        }else{
          fore <- c('', predictions)
          fore <- diffinv(fore, lag=input$seas, differences=1, 
                          xi=data[1:(input$seas)])
        }
      }else if(input$diff == TRUE){
        if(input$diff2 == TRUE){
          fore <- c('', predictions)
    ...
    df %>% ggplot(aes(x=time)) + 
      ...
      geom_line(aes(y=observation, color="observation")) +
      geom_vline(xintercept = time[(length(data())-12)], linetype="dotted") + 
      ggtitle("Your Forecasts") + labs(y = ylabel()) + 
      scale_color_manual(values = c("blue", "red")) +
      labs(x = ifelse(input$sets=="internet", "Time, in Hours", 
                      sprintf("Time, in years (recall, there are %d 
                              observations per year in this data set)", 
                              frequency(data())))) +
      theme(legend.title=element_blank(), 
            axis.title.y = element_text(angle = 45, color = "blue"), 
  ...
  almost1 <- ''#youre just missing something very menial, the 
  # important stuff is there
  almost2 <- ''#arima order very close
  
  success <- "Great job!"
  notyet1 <- "Try improving the stationarity of your model"
  notyet2 <- "Try a new value for the period, or frequency for 
  the seasonality in your model"
  notyet3 <- "Consult the acf plots and the armasubsets and try 
  fitting better coefficients"
  ...
  feed <- reactive({
    ...
    if(input$sets == "internet"){
      if(my_if(model[[2]]==c(1,2,1)) & my_if(model[[3]]==c(2,1,0)) & 
         model[[4]]==24 & model[[5]]==0 & model[[1]]=="yes"){
        success
      }else if(my_if(model[[2]]==c(2,2,1)) & my_if(model[[3]]==c(2,1,0)) & 
               model[[4]]==24 & model[[5]]==0){
        almost # or, theyre good; success
      }else if(sum(model[[2]][2]!=2, model[[3]][2]!=1, model[[4]]!=24, 
                   model[[5]]!=0)>0){
        if(sum(model[[2]][2]!=2, model[[3]][2]!=1, 
               model[[5]]!=0)==0 & model[[4]]!=24){
          notyet2
        }else{
          notyet1
        }
      ...
      if(model[[5]] == 0){
        if(model[[4]] == 12){
          if(model[[1]]=="yes" & (model[[2]][2]+model[[3]][2]==3)){
            if(model[[2]][2]==1){
              "You're close, but the data could be differenced a little better 
        ...
        if(when()=="last" | 
            sum(
              input$log == FALSE, input$diff == FALSE, 
              input$seas_diff == FALSE, when()=="first")==4){
      ...
      if(model[[1]]=="yes" & model[[2]][2]==1 & model[[5]]==0 & 
         model[[3]][2]==0){
        if(model[[4]] == 7){
      ...
      if(model[[1]]=="no" & model[[2]][2]==1 & model[[5]]==0 & 
         model[[3]][2]==0 & model[[4]]==0){
        if(model[[4]] == 0){
      ...
      if(model[[1]]=="no" & model[[2]][2]==0 & model[[3]][2]==1 & model[[5]]==0){
        if(model[[4]] == 12){
          if(my_if(model[[3]]==c(0,1,1)) & 
              sum(
                my_if(model[[2]]==c(9,0,0)), 
                my_if(model[[2]]==c(3,0,1)))==2){
            success
          }else{
            notyet3
          }
      ...
      if(when()=="first"){
        if(model[[1]]=="no" & model[[2]][2]==0 & model[[3]][2]==0 & 
           model[[5]] %in% c(2,3)){
  ...
  output$bar <- renderImage({
    ...
    image(1:place, 1:10, matrix(9, place, 10), asp=1, xlim=c(0,100), 
          ylim=c(0,57),
